...
@property (nonatomic, assign) int matchingFrames;
@property (nonatomic, assign) int minMatchingFrames;
@property (nonatomic, retain) CIRectangleFeature* previousRectangle;
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (self.isBorderDetectionEnabled)
    {
        ...
        if (_borderDetectLastRectangleFeature)
        {
            ...
            if (rectangleDetectionConfidenceHighEnough(_imageDedectionConfidence) && !self.captureDevice.isAdjustingFocus) {
                BOOL steadyRectangle = YES;
                if (_previousRectangle) {
                    steadyRectangle = [self compareRect:_previousRectangle withRect:_borderDetectLastRectangleFeature];
                }
                _matchingFrames = steadyRectangle ? _matchingFrames + 1 : 0;
            } else {
                _matchingFrames = 0;
            }
        } else {
        ...
        _matchingFrames = _matchingFrames >= 0 ? _matchingFrames : 0;
        float progress = (float)_matchingFrames / (float)_minMatchingFrames;
        [self.delegate autoCaptureProgress:progress];
        _previousRectangle = _borderDetectLastRectangleFeature;
        if (_matchingFrames >= _minMatchingFrames) {
            self.forceStop = YES;
            [self captureImage];
            return;
        }
    ...
    if (self.context && _coreImageContext)
    {
        ...
        float aspectRatio = image.extent.size.width / image.extent.size.height;
        CGSize previewSize = CGSizeMake(self.bounds.size.width, self.bounds.size.width / aspectRatio);
        CGPoint previewOrigin = CGPointMake(0, self.bounds.origin.y + ((self.bounds.size.height - previewSize.height) / 2));
        CGRect rect = CGRectMake(previewOrigin.x, previewOrigin.y, previewSize.width, previewSize.height);
        [_coreImageContext drawImage:image inRect:rect fromRect:image.extent];
        [_glkView display];
...
- (BOOL)compareRect:(CIRectangleFeature *)rect1 withRect:(CIRectangleFeature *)rect2
{
    return [self comparePoint:rect1.topLeft withPoint:rect2.topLeft] &&
        [self comparePoint:rect1.topRight withPoint:rect2.topRight] &&
        [self comparePoint:rect1.bottomLeft withPoint:rect2.bottomLeft] &&
        [self comparePoint:rect1.bottomRight withPoint:rect2.bottomRight];
}

- (BOOL)comparePoint:(CGPoint)point1 withPoint:(CGPoint)point2
{
    CGFloat padding = 30;
    CGRect rect1 = CGRectMake(point1.x - padding,
                              point1.y - padding,
                              padding * 2,
                              padding * 2);
    return CGRectContainsPoint(rect1, point2);
}
...
{
    CIImage *overlay = [CIImage imageWithColor:[CIColor colorWithRed:0.2 green:0.57 blue:1 alpha:0.4]];
...
//                 CFRelease(imgRef);
             ...
             {
                [self.delegate takeSnapshotWithPath:filePath];
                dispatch_resume(_captureQueue);
             });
...
- (CIDetector *)rectangleDetetor
{
    ...
    {
        detector = [CIDetector detectorOfType:CIDetectorTypeRectangle context:nil options:@{CIDetectorAccuracy : CIDetectorAccuracyLow,CIDetectorTracking : @(YES), CIDetectorAspectRatio: @(_targetAspectRatio), CIDetectorMinFeatureSize: [NSNumber numberWithFloat:0.3]}];
    });
    return detector;
}
    ...
    dispatch_once(&onceToken, ^
    {
        detector = [CIDetector detectorOfType:CIDetectorTypeRectangle
                                      ...
                                      options:@{
                                                ...
                                                CIDetectorAspectRatio: @(_targetAspectRatio)
                                                }];
    });
    return detector;
}
